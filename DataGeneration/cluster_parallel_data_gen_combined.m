function [] = cluster_parallel_data_gen_combined(signals_per_SNR, resize_method, transform, num_workers, seed, train, sigma)
    ...
    for snr_index = 1:length(SNR)
         prefix_clean = fullfile(output_dir, [prefix resize_method '_' transform '_' num2str(snr_index) '_' 'sigma' '_' num2str(sigma) '_combined.h5']);
         h5create(prefix_clean, '/vsst_images/images_real', [image_size, image_size, total_signals_per_SNR], 'Datatype', 'double');
         h5create(prefix_clean, '/vsst_images/images_imag', [image_size, image_size, total_signals_per_SNR], 'Datatype', 'double');
         h5create(prefix_clean, '/cwd_images', [image_size, image_size, total_signals_per_SNR], 'Datatype', 'double');
            ...
            switch waveform
                case 'LFM'
                    ...
                    for idx = 1:signals_per_SNR
                        ...
                        resized_images = transform_data_combined(wav, noisy_signal, 1024, image_size, real_noise_std, resize_method, transform, sigma);
...
function write_batch_to_h5(prefix_clean, input_batch, input_noisy_batch, output_data_batch, start, image_size, signals_per_SNR)
    ...
    h5write(prefix_clean, '/vsst_images/images_real', real(input_batch), [1 1 start], [image_size, image_size, signals_per_SNR]);
    h5write(prefix_clean, '/vsst_images/images_imag', imag(input_batch), [1 1 start], [image_size, image_size, signals_per_SNR]);
    h5write(prefix_clean, '/cwd_images', real(input_noisy_batch), [1 1 start], [image_size, image_size, signals_per_SNR]);
...
function combine_h5_files(resize_method, transform, train, snr_length, signals_per_SNR, num_waveforms, image_size, output_dir, sigma)
    ...
    combined_clean_file = fullfile(output_dir, [prefix resize_method '_' transform '_' 'sigma' '_' num2str(sigma) '_combined.h5']);

    h5create(combined_clean_file, '/vsst_images/images_real', [image_size, image_size, total_signals], 'Datatype', 'double');
    h5create(combined_clean_file, '/vsst_images/images_imag', [image_size, image_size, total_signals], 'Datatype', 'double');
    h5create(combined_clean_file, '/cwd_images', [image_size, image_size, total_signals], 'Datatype', 'double');
    ...
    for snr_index = 1:snr_length
        ...
        for waveform_index = 1:num_waveforms
            ...
            real_data = h5read(prefix_clean, '/vsst_images/images_real', [1,1, batch_start_index], [image_size,image_size, num_signals_to_read]);
            imag_data = h5read(prefix_clean, '/vsst_images/images_imag', [1,1, batch_start_index], [image_size,image_size, num_signals_to_read]);
            h5write(combined_clean_file, '/vsst_images/images_real', real_data, [1,1,current_index], [image_size,image_size, num_signals_to_read]);
            h5write(combined_clean_file, '/vsst_images/images_imag', imag_data, [1,1,current_index], [image_size,image_size, num_signals_to_read]);
            
            %Read and write noisy data
            real_data_noisy = h5read(prefix_clean, '/cwd_images', [1,1, batch_start_index], [image_size,image_size, num_signals_to_read]);
            h5write(combined_clean_file, '/cwd_images', real_data_noisy, [1,1,current_index], [image_size,image_size, num_signals_to_read]);
