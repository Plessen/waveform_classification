function [] = cluster_parallel_data_gen_cwd(signals_per_SNR, resize_method, transform, num_workers, seed, train, sigma)
    ...
    for snr_index = 1:length(SNR)
         ...
         h5create(prefix_clean, '/noisy_images', [image_size, image_size, total_signals_per_SNR], 'Datatype', 'double');
            ...
            switch waveform
                case 'LFM'
                    ...
                    for idx = 1:signals_per_SNR
                        ...
                        resized_images = transform_data_cwd(wav, noisy_signal, 1024, image_size, real_noise_std, resize_method, transform, sigma);
                         
                        input_noisy_batch(:, :,idx) = resized_images.transform_noisy_resized;
                    end
                    write_batch_to_h5(prefix_clean, input_noisy_batch, output_data_batch, start_index, image_size, signals_per_SNR)
...
function write_batch_to_h5(prefix_clean, input_noisy_batch, output_data_batch, start, image_size, signals_per_SNR)
    input_noisy_batch = permute(input_noisy_batch, [2, 1, 3]);
    h5write(prefix_clean, '/noisy_images', input_noisy_batch, [1 1 start], [image_size, image_size, signals_per_SNR]);
...
function combine_h5_files(resize_method, transform, train, snr_length, signals_per_SNR, num_waveforms, image_size, output_dir, sigma)
    ...
    h5create(combined_clean_file, '/noisy_images', [image_size, image_size, total_signals], 'Datatype', 'double');
    ...
    for snr_index = 1:snr_length
        ...
        for waveform_index = 1:num_waveforms
            ...
            real_data_noisy = h5read(prefix_clean, '/noisy_images', [1,1, batch_start_index], [image_size,image_size, num_signals_to_read]);
            h5write(combined_clean_file, '/noisy_images', real_data_noisy, [1,1,current_index], [image_size,image_size, num_signals_to_read]);
