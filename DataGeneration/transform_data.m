function resized_images = transform_data(signal, noisy_signal, N, image_size, real_noise_std, resize_method, transform)
    ...
    [~, SST_noisy, VSST_noisy, ~, ~, ~, ~] = sst2_new(noisy_signal, 1 / sigma^2 / N, N, real_noise_std);
    SST_noisy = SST_noisy(:, 1:original_size);
    VSST_noisy = VSST_noisy(:, 1:original_size);
  

    % Compute SST and VSST for clean signal
    [~, SST_clean, VSST_clean, ~, ~, ~, ~] = sst2_new(signal, 1 / sigma^2 / N, N, 0);
    SST_clean = SST_clean(:, 1:original_size);
    VSST_clean = VSST_clean(:, 1:original_size);
    ...
    % Resize SST and VSST for the noisy signal
    %resized_images.SST_noisy_resized = imresize(SST_noisy, [image_size, image_size], resize_method, "Antialiasing", false);
    %resized_images.VSST_noisy_resized = imresize(VSST_noisy, [image_size, image_size], resize_method, "Antialiasing", false);

    % Resize SST and VSST for the clean signal
    %resized_images.SST_resized = imresize(SST_clean, [image_size, image_size], resize_method, "Antialiasing", false);
    %resized_images.VSST_resized = imresize(VSST_clean, [image_size, image_size], resize_method, "Antialiasing", false);
end
