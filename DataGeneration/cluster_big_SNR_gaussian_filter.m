function [] = cluster_big_SNR_gaussian_filter(signals_per_SNR, resize_method, transform, seed, train, sigma, image_size_row, image_size_col)
    ...
    for snr_index = 1:length(SNR)
         prefix_clean = fullfile(output_dir, [prefix resize_method '_' transform '_' num2str(snr_index) '_' 'sigma' '_' num2str(sigma) '_big_snr_' num2str(image_size_row) '-' num2str(image_size_col) '-filter.h5']);
...
function write_batch_to_h5(prefix_clean, input_batch, output_data_batch, start_index, image_size_row, image_size_col, signals_per_SNR)
    input_batch = permute(input_batch, [2, 1, 3]);
    h5write(prefix_clean, '/noisy_images/images_real', real(input_batch), [1 1 start_index], [image_size_col, image_size_row, signals_per_SNR]);
    h5write(prefix_clean, '/noisy_images/images_imag', imag(input_batch), [1 1 start_index], [image_size_col, image_size_row, signals_per_SNR]);
    h5write(prefix_clean, '/labels', output_data_batch, [1 start_index], [size(output_data_batch, 1) signals_per_SNR]);
end

function combine_h5_files(resize_method, transform, train, snr_length, signals_per_SNR, num_waveforms, image_size_row, image_size_col, output_dir, sigma)
    ...
    combined_clean_file = fullfile(output_dir, [prefix resize_method '_' transform '_' 'sigma' '_' num2str(sigma) '_big_snr_' num2str(image_size_row) '-' num2str(image_size_col) '-filter.h5']);
    ...
    for snr_index = 1:snr_length
        ...
        for waveform_index = 1:num_waveforms
            ...
            real_data = h5read(prefix_clean, '/noisy_images/images_real', [1,1, batch_start_index], [image_size_col, image_size_row, num_signals_to_read]);
            imag_data = h5read(prefix_clean, '/noisy_images/images_imag', [1,1, batch_start_index], [image_size_col, image_size_row, num_signals_to_read]);
            h5write(combined_clean_file, '/noisy_images/images_real', real_data, [1,1,current_index], [image_size_col, image_size_row, num_signals_to_read]);
            h5write(combined_clean_file, '/noisy_images/images_imag', imag_data, [1,1,current_index], [image_size_col, image_size_row, num_signals_to_read]);
